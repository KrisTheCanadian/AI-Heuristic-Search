...
public static class Validator
{
    private static int Inversions(Puzzle puzzle)
    {
        ...
        foreach (var row in flattenedPuzzle)
        {
            flattenedPuzzleList.AddRange(row);
        }
        ...
        for (var i = 0; i < flattenedPuzzleList.Count; i++)
        {
            ...
            {
                if (flattenedPuzzleList[i] != 0 && flattenedPuzzleList[j] != 0 && flattenedPuzzleList[i] > flattenedPuzzleList[j])
                {
                    inversions++;
                }
            }
